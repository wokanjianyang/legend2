...
namespace Game
{
    ...
    public struct Offset : IEquatable<Offset>
    {
        ...
        {
            if (x < -1 || x > 1) throw new ArgumentOutOfRangeException(nameof(x), $"Parameter {nameof(x)} cannot have a magnitude larger than one");

            if (y < -1 || y > 1) throw new ArgumentOutOfRangeException(nameof(y), $"Parameter {nameof(y)} cannot have a magnitude larger than one");

            if (x == 0 && y == 0) throw new ArgumentException(nameof(y), $"Paramters {nameof(x)} and {nameof(y)} cannot both be zero");
