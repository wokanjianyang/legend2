...
namespace Game
{
    ...
    internal static partial class PathFinder
    {
        public static List<Position> FindPath(Grid grid, Position start, Position end, Offset[] movementPattern)
        {
            ...
            ClearStepList();
            //
            if (start == end) return new List<Position> {start};
            ...
            while (open.HasNext())
            {
                ...
                if (current == end) return ReconstructPath(grid, start, end, cameFrom);
        ...
        private static void Step(Grid grid, MinHeap open, Position[] cameFrom, float[] costSoFar, Offset[] movementPattern, Position current, Position end)
        {
        ...
        private static IEnumerable<Offset> GetMovementOptions(Position position, int dimX, int dimY, IEnumerable<Offset> movementPattern)
        {
            return movementPattern.Where(m =>
            {
