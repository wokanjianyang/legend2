...
public class ProGifPlayerPanel : MonoBehaviour
{
	...
	public enum DataPathTypeDebug
	{
		...
		/// Combine Application.streamingAssetsPath and fileName(name.gif) from input field.
		/// (To test your gif, put your gif in the Assets/StreamingAssets folder of the project, input the gif fileName in the url input field)
		StreamingAssetsPath,
	}
	...
	public void SaveToGallery()
	{
        ...
        StartCoroutine(filePathName.LoadFileUWR(gifPath,
            (data) =>
            {
                if (data != null)
                {
                    MobileMedia.SaveBytes(data, "Pro GIF", filePathName.GetFileNameWithoutExt(true), ".gif", true);
                }
            })
        );
#else
	...
	public void Play()
	{
		...
		case DataPathTypeDebug.PersistentDataPath:
			gifPath = System.IO.Path.Combine(Application.persistentDataPath, gifPath);
			if(!gifPath.ToLower().Contains(".gif")) gifPath += ".gif";
			break;
...
//				}
    ...
    public void Example_ChangePlaybackSpeed(float speed)
    {
        ProGifPlayer proGifPlayer = ProGifManager.Instance.m_GifPlayer;
        proGifPlayer.playerComponent.playbackSpeed = speed;
    }
}
