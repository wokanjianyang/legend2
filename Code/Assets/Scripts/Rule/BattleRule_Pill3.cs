...
public class BattleRule_Pill3 : ABattleRule
{
    ...
    public override void DoMapLogic(int roundNum, double currentRoundTime)
    {
        ...
        if (enemys.Count <= 0)
        {
            ...
            GameProcessor.Inst.EventCenter.Raise(new BattleMsgEvent() { Type = RuleType.Pill3, Message = "挑战通关！" });
    ...
    public override void CheckGameResult()
    {
        ...
        if (heroCamp.HP <= 0)
        {
            GameProcessor.Inst.EventCenter.Raise(new BattleMsgEvent() { Type = RuleType.Pill3, Message = "挑战失败！" });
